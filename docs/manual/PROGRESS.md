...
| — (stage 3: 42 YouTube tutorials, batches 1–13) | Third pass against the 42 public tutorial videos (see docs/video-coverage/VIDEO_COVERAGE_YT.md). Batches 1–5: pinnable toolbar quick actions + sidebar collapse F9 (§5.1), Settings → Views display preferences, tooth numbers on implants, Shift+wheel zoom (§5.2), free 3D model import, per-model Look/Properties/Adjust position incl. Size ±5 %, Create merged model, measurement rename (§5.3), Copy-plan element selection (§5.4), nerve rename + ✕ Delete point (§6.3), import-time mesh-optimize offer + Fine-align Size section (§6.4), implant position lock / display color / fine-positioning panel / virtual-tooth wax-up models (§6.5), elongated (stadium) inspection windows (§6.6), Mesh Editor additions — Partial repair, Combine Subtract + show-object preview, Select-area smoothing, drag-to-paint, Ctrl+wheel radius, double-click pivot, view-type switch, margin-spline editing, Remesh max-edge/strength, Reduce guidance (§6.8). Batches 6–13: AI tooth renumbering in the review wizard's FDI chart, wizard subset quick-picks + live rail counts + scan-step Manual alignment…/Reset view, Tooth extraction dialog (§6.4 + §5.3), workflow-step hiding + extended quick-action catalog + title-row ⚙/⌨ (§5.1), zoom readout, 3D double-click turning point, default implant color setting (§5.2), X-ray model look, triangles·points·volume·surface·dimensions Properties, Edit mesh… on every model kind, 3D volume render + Implants through surfaces toggles (§5.3), bulk Lock implants (§5.4), nerve detection waypoint choice (§6.3), mask-editor upper HU bound (§6.4), padlock in tree + 0.1° fine rotation step (§6.5), Undercuts insertion preview + label presets Date of birth/Tooth positions + transplant flow via Tooth extraction (§6.6), practice logo PNG/JPEG/BMP/WebP (§6.7), to-scale scanbody thumbnails (§6.9), cross/tangential-view 3D measurements + armed-tool copy cursor (§7.3) | §5.1–§5.4, §6.3–§6.9, §7.3 | yt-settings-views.png, yt-implant-toolbar.png, yt-merge-models.png | in progress (pass ongoing; batches 1–13 documented) |
...
the features of build batches 1–13 — including AI tooth renumbering and AI tooth
extraction — are documented above.
