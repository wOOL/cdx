/**
 * Build docs/manual/DWOS-Web-Manual.pdf from the DWOS Web chapter markdown.
 *
 * Renders a standalone, light-themed print document so the PDF is clean in every
 * viewer. Clone of scripts/build-manual-pdf.ts pointed at docs/manual-dwos.
 * Tolerant of zero images (the DWOS manual ships its figures during a later
 * screenshot pass; the img/ dir may contain only .gitkeep).
 * Run: `bun run scripts/build-dwos-manual-pdf.ts`
 */
...
const DIR = join(import.meta.dir, '..', 'docs', 'manual-dwos');
const OUT = join(import.meta.dir, '..', 'docs', 'manual', 'DWOS-Web-Manual.pdf');
const files = readdirSync(DIR)
	.filter((f) => /^\d{2}-.*\.md$/.test(f))
	.sort();

/** Inline every img/foo.png as a base64 data URI — avoids any file:// / network
 *  image-loading race (the images become part of the HTML itself). When the
 *  manual ships no images yet, this simply makes no replacements. */
...
try {
	...
	// confirm they all decoded (hard 30 s cap so this can never hang). With zero
	// images this resolves immediately as 0/0.
	...
	await page.pdf({
		...
		footerTemplate:
			'<div style="width:100%;font-size:8px;color:#888;text-align:center">DWOS Web — User Manual · page <span class="pageNumber"></span> / <span class="totalPages"></span></div>',
